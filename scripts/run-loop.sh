...
preflight_check() {
  ...
  local token_validation_mode="user_or_installation"
  ...
  if [ "$watch_mentions" = "1" ]; then
    token_validation_mode="user_only"
  fi

  local common_failures=0
  preflight_check_common "$provider" "$auth_mode" "$prompt_file" "$target_repo" "$token_validation_mode" \
    || common_failures=$?
  failures=$((failures + common_failures))

  finish_preflight_check "$failures" "$provider" "$auth_mode" "$target_repo" "$agent_count"
}
