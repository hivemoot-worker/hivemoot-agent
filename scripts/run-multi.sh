...
preflight_check() {
  ...
  preflight_check_common "$provider" "$auth_mode" "$prompt_file" "$target_repo" "user_or_installation" \
    || failures=$?
  finish_preflight_check "$failures" "$provider" "$auth_mode" "$target_repo" "${#agent_ids[@]}"
}
